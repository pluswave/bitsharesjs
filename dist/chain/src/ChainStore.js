            ...
            return db_api.exec("get_objects", [["2.1.0"]]).then(function (optional_objects) {
                ...
                for (var i = 0; i < optional_objects.length; i++) {
                    ...
                    if (optional_object) {
                        ...
                        if (delta < 60) {
                            _bitsharesjsWs.Apis.instance().db_api().exec("set_subscribe_callback", [_this.onUpdate.bind(_this), true]).then(function () {
        ...
        if (current === undefined) {
            ...
            */
            ...
            _bitsharesjsWs.Apis.instance().db_api().exec("get_balance_objects", [[address]]).then(function (balance_objects) {
                ...
                _this5.balance_objects_by_address.set(address, _immutable2.default.Set(set));
                _this5.notifySubscribers();
            }, function () {
                _this5.balance_objects_by_address.delete(address);
            });
        }
            ...
            _bitsharesjsWs.Apis.instance().db_api().exec("get_objects", [[id]]).then(function (optional_objects) {
                ...
                for (var _i = 0; _i < optional_objects.length; _i++) {
                    ...
                    if (optional_object) _this6._updateObject(optional_object, true);else {
                        _this6.objects_by_id.set(id, null);
                ...
                // in the event of an error clear the pending state for id
                console.log("!!! Chain API error", error);
                _this6.objects_by_id.delete(id);
            });
        ...
        return new Promise(function (resolve, reject) {
            _bitsharesjsWs.Apis.instance().db_api().exec("get_witness_by_account", [account_id]).then(function (optional_witness_object) {
                if (optional_witness_object) {
                    ...
                    var witness_object = _this7._updateObject(optional_witness_object, true);
                    resolve(witness_object);
                } else {
                    _this7.witness_by_account_id = _this7.witness_by_account_id.set(account_id, null);
        ...
        return new Promise(function (resolve, reject) {
            _bitsharesjsWs.Apis.instance().db_api().exec("get_committee_member_by_account", [account_id]).then(function (optional_committee_object) {
                if (optional_committee_object) {
                    ...
                    var committee_object = _this8._updateObject(optional_committee_object, true);
                    resolve(committee_object);
                } else {
                    _this8.committee_by_account_id = _this8.committee_by_account_id.set(account_id, null);
    ...
    */
    ...
    ChainStore.prototype.fetchFullAccount = function fetchFullAccount(name_or_id) {
        ...
        if (!this.fetching_get_full_accounts.has(name_or_id) || Date.now() - this.fetching_get_full_accounts.get(name_or_id) > 5000) {
            ...
            _bitsharesjsWs.Apis.instance().db_api().exec("get_full_accounts", [[name_or_id], true]).then(function (results) {
                if (results.length === 0) {
                    if (_ChainValidation2.default.is_object_id(name_or_id)) {
                ...
                _this9._subTo("accounts", full_account.account.id);
                ...
                _this9.accounts_by_name.set(account.name, account.id);
                ...
                _this9.fetchRecentHistory(updated_account);
                _this9.notifySubscribers();
            }, function (error) {
                console.log("Error: ", error);
                if (_ChainValidation2.default.is_object_id(name_or_id)) _this9.objects_by_id.delete(name_or_id);else _this9.accounts_by_name.delete(name_or_id);
            });
        }
        return undefined;
    };
    ...
    ChainStore.prototype.fetchRecentHistory = function fetchRecentHistory(account) {
        ...
        pending_request.promise = new Promise(function (resolve, reject) {
            _bitsharesjsWs.Apis.instance().history_api().exec("get_account_history", [account_id, most_recent, limit, start]).then(function (operations) {
                ...
                _this10.objects_by_id.set(account_id, updated_account);

                //if( current_history != updated_history )
                //   this._notifyAccountSubscribers( account_id )

                var pending_request = _this10.account_history_requests.get(account_id);
                _this10.account_history_requests.delete(account_id);
                if (pending_request.requests > 0) {
                    ...
                    // this query.
                    _this10.fetchRecentHistory(updated_account, limit).then(resolve, reject);
    ...
    */
            ...
            _bitsharesjsWs.Apis.instance().db_api().exec("lookup_vote_ids", [missing]).then(function (vote_obj_array) {
                ...
                for (var _i2 = 0; _i2 < vote_obj_array.length; ++_i2) {
                    if (vote_obj_array[_i2]) {
                        ...
                        _this11._subTo(isWitness ? "witnesses" : "committee", vote_obj_array[_i2].id);
                        _this11._updateObject(vote_obj_array[_i2]);
                    }
                }
            }, function (error) {
    ...
    ChainStore.prototype.addProposalData = function addProposalData(approvals, objectId) {
        ...
        approvals.forEach(function (id) {
            var impactedAccount = _this12.objects_by_id.get(id);
            if (impactedAccount) {
                ...
                if (!proposals.includes(objectId)) {
                    ...
                    _this12._updateObject(impactedAccount.toJS());
                }
            }
        });
    };
